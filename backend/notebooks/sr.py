...
def get_trades_from_signal(data: pd.DataFrame, signal: np.array):
    ...
    long_trades['percent'] = (long_trades['exit_price'] - long_trades['entry_price']) / long_trades[
        'entry_price'
    ]
    short_trades['percent'] = (
        -1
...
if __name__ == '__main__':
    ...
    file = os.path.join('', 'nifty-dataset.json')
    ...
    print(
        f"SR Strategy Return: {data['sr_return'].sum()*100:.2f}%, Buy and Hold Return: {data['log_ret'].sum()*100:.2f}%"
    )
