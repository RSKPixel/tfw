...
def main():
    ...
    while True:
        ...
        if first_run:
            try:
                period = int(input("Initial run - Enter number of days to backfill: 0-31: "))
            except ValueError:
                period = 1
        ...
        historicals(exchange='nfo', period=period, interval='minute', api=kite, conn=conn)
...
def wait_until_next(waiting_minutes=1, seconds=1):
    ...
    if next_minute == 60:
        next_run = now.replace(hour=(now.hour + 1) % 24, minute=0, second=seconds, microsecond=0)
    else:
        next_run = now.replace(minute=next_minute, second=seconds, microsecond=0)
    ...
    try:
        while True:
            ...
            print(f"\r⏳ Sleeping... {mins:02d}m {secs:02d}s remaining", end="", flush=True)
            time.sleep(1)
